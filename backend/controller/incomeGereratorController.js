...
export const generateReferalIncome = async (
  ...
  if (sponser1.isAdmin == false) {
    ...
    console.log(
      `(5)sponser1 ${sponser1.name} direct referal=== ${directReferalIncome}`
    );
  ...
  if (sponser2 && sponser2.isAdmin == false) {
    ...
    console.log(
      `sponser2 ${sponser2.name} direct referal=== ${directReferalIncome}`
    );
...
export const levelIncomeGenerator = async (userData, amount) => {
  ...
  console.log(
    `enter to level income function---${userData.name}--Amount is-----${amount}-----------------`
  );
  while (amount >= 10) {
    ...
    console.log(
      `${updatedSponser.name} has credited level income ${levelIncome}`
    );
    if (updatedSponser) {
      await franchiseIncomeGenerator(
        updatedSponser,
...
export const franchiseIncomeGenerator = async (
  userData,
  amount,
  name,
  distPercentage,
  ...
  try {
    ...
    console.log(
      `District franchise income credited to ${districtData.name} amount is ${districtIncome}`
    );
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `Zonal franchise income credited to ${zonalData.name} amount is ${zonalIncome}`
    );
  } catch (error) {
    console.error("Error saving District data:", error);
  }
...
export const countTeamMembers = async (userId) => {
  try {
    ...
    let count =
      (user.childLevel1 ? user.childLevel1.length : 0) +
      (user.childLevel2 ? user.childLevel2.length : 0);
...
export const generatePromotersIncome = async (
  amount,
  userData,
  promoterPercentage
) => {
...
export const addPoolPercentage = async (req, res, next) => {
  try {
    ...
    adminData.autoPoolPercentageA =
      autoPoolPercentageA || adminData.autoPoolPercentageA;
    adminData.autoPoolPercentageB =
      autoPoolPercentageB || adminData.autoPoolPercentageB;
    adminData.autoPoolPercentageC =
      autoPoolPercentageC || adminData.autoPoolPercentageC;
    adminData.autoPoolPercentageD =
      autoPoolPercentageD || adminData.autoPoolPercentageD;
    adminData.autoPoolPercentageE =
      autoPoolPercentageE || adminData.autoPoolPercentageE;
...
export const distributeAutoPoolWallet = async (req, res, next) => {
  try {
    ...
    const poolACount = await distributeToPool(
      adminData.poolA,
      amountPoolA,
      autoPoolPercentageA
    );
    const poolBCount = await distributeToPool(
      adminData.poolB,
      amountPoolB,
      autoPoolPercentageB
    );
    const poolCCount = await distributeToPool(
      adminData.poolC,
      amountPoolC,
      autoPoolPercentageC
    );
    const poolDCount = await distributeToPool(
      adminData.poolD,
      amountPoolD,
      autoPoolPercentageD
    );
    const poolECount = await distributeToPool(
      adminData.poolE,
      amountPoolE,
      autoPoolPercentageE
    );
    ...
    let adminShare=0;
      if(poolACount>0){
        adminShare=(amountPoolA/poolACount)
      }else{
        adminShare=amountPoolA
      }

      if(poolBCount>0){
        adminShare+=(amountPoolB/(poolBCount+1))
      }else{
        adminShare+=amountPoolB
      }

      if(poolCCount>0){
        adminShare+=(amountPoolC/(poolCCount+1))
      }else{
        adminShare+=amountPoolC
      }

      if(poolDCount>0){
        adminShare+=(amountPoolD/(poolDCount+1))
      }else{
        adminShare+=amountPoolD
      }

      if(poolECount>0){
        adminShare+=(amountPoolE/(poolECount+1))
      }else{
        adminShare+=amountPoolE
      }


      adminData.shareFromPool=adminShare;
      adminData.totalShareFromPool+=adminShare;
