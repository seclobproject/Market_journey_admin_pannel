...
export const walletWithdrawReportUser = async (req, res, next) => {
  try {
    ...
    const userData = await User.findById(userId)
      .populate("walletWithdrawHistory")
      .sort({ createdAt: 1 });
...
const paginateAggregate = async (
  ...
  try {
    ...
    const totalDocs =
      totalDocsCountResult.length > 0 ? totalDocsCountResult[0].totalDocs : 0;
    const totalPages = Math.ceil(totalDocs / pageSize);
...
export const userDemateAccounts = async (req, res, next) => {
  try {
    ...
    if (userData) {
      ...
      const paginateduserDemateAccounts = await paginate(
        userDemateAccounts,
        page,
        pageSize
      );
  ...
  try {
    ...
    if (["poolA", "poolB", "poolC", "poolD", "poolE"].includes(pool)) {
      ...
      if (userData) {
        ...
        const userStatus = userData.userStatus;
        const paginatedPoolUsers = await paginate(
          poolUsers,
          parseInt(page),
          parseInt(pageSize)
        );
...
export const getAutoPoolCountAmount = async (req, res, next) => {
  try {
    ...
    const latestDistribution =
      autoPoolDistributionHistory[autoPoolDistributionHistory.length - 1];
...
export const bonusPaidReportPaginated = async (req, res, next) => {
  try {
    ...
    if (userData) {
      ...
      const paginatedaddBounusHistory = await paginate(
        addBounusHistory,
        page,
        pageSize
      );
...
//Demate accounts filtered view

export const filteredDemates = async (req, res, next) => {
  try {
    const userId = req.admin ? req.admin._id : req.user ? req.user._id : null;
    let page = parseInt(req.query.page) || 1; // Default to page 1 if not provided
    const pageSize = parseInt(req.query.pageSize) || 10; // Default page size to 10 if not provided
    ...
    if (demates) {
      ...
      res.status(200).json({
        ...
        pagination: {
          page: paginatedDemates.page,
          pageSize: paginatedDemates.pageSize,
          totalPages: paginatedDemates.totalPages,
          totalDocs: paginatedDemates.totalDocs,
        },
        sts: "01",
        msg: "Get Filtered Demates",
      });
    } else {
      return next(errorHandler(401, "No Members"));
    }
...
export const viewPendingRenewalsPaginated = async (req, res, next) => {
  try {
    const userId = req.admin._id;
    const adminData = await Admin.findById(userId);
    if (!adminData) {
    ...
    const userData = await User.find({ subscriptionStatus: "pending" }).select(
      "name email phone screenshot tempPackageAmount pendingPackage action sponserName updatedAt"
    );

    const paginatedPendingRenewals = await paginate(userData, page, pageSize);

    res.status(200).json({
      pendingRenews: paginatedPendingRenewals.results,
      pagination: {
        page: paginatedPendingRenewals.page,
        pageSize: paginatedPendingRenewals.pageSize,
        totalPages: paginatedPendingRenewals.totalPages,
        totalDocs: paginatedPendingRenewals.totalDocs,
      },
      sts: "01",
      msg: "Get Pending Renewal request report ",
    });
  } catch (error) {
    next(error);
  }
