...
import { districtPageUrl, districtlistPageUrl, statelistPageUrl } from "../../../utils/Constants";
  ...
  <div className="card-body p-2">
    <div className="table-container table-responsive rounded-2 mb-4">
      <table className="table border text-nowrap customize-table mb-0 align-middle">
        <thead className="text-dark fs-4" style={{ backgroundColor: 'yellow' }}>
          <tr>
            <th>
          ...
          {districtList?.length ? (
            <>
              {districtList.map((districts, index) => (
                <tr key={index}>
                  <td>{index + 1}</td>
                  <td>{districts?.name && districts.name.toUpperCase() || "--"}</td>
                  <td>{districts?.stateName && districts.stateName.toUpperCase() || "--"}</td>
                  <td>{districts?.packageAmount && districts.packageAmount || "--"}</td>
                </tr>
              ))}
            </>
          ) : (
